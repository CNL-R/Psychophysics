...
for block = 1:blocks
    ...
    if trial == 1
        ...
        if blockMatrix(1, block) == 1
            vbl = PresentStimulus(cueTexture, window, vbl, ifi, cueTime, 0, false, 0, 0, rectCenter);
        elseif blockMatrix(1, block) == 2
            PresentBeep(cueFrequency, cueTime, nrchannels, volume)
    ...
    while run <= runs
        ...
        %Presenting Cue
        if blockMatrix(1, block) == 3
            vbl = PresentStimulus(cueTexture, window, vbl, ifi, cueTime, 0, false, 0, 0, rectCenter);
        elseif blockMatrix(1, block) == 2
            PresentBeep(cueFrequency, cueTime, nrchannels, volume)
        elseif blockMatrix(1, block) == 1
            PresentAVStimulus(cueTexture, cueFrequency, volume, window, vbl, ifi, cueTime, 0, 0, 0, 0, rectCenter);
        end
