...
%creating apperatures
apperatures = zeros(length, width, numTextures);
% for i = 1:numTextures
%     apperatures(:,:,i) = CreateApperature('r', length, width, 'n', 0.5);
% end
gabor = CreateGabor(gaborWidth, sigma, lambda, 'r', 'r', A);
noised_gabor = EmbedInNoise(gabor, coherence, 0, gauss);
stim = EmbedInEfficientApperature(noised_gabor, noise(:,:, round(rand(1) * (numTextures - 1) + 1)));
stimTexture = Screen('MakeTexture', window, stim);
...
% Playing Back Gabors
Screen('DrawTextures', window, stimTexture(1) ,[], rectCenter, [], [], [], []);
vbl = Screen('Flip', window);
for frame = 1:timeFrames - 1
    Screen('DrawTextures', window, stimTexture(frame + 1), [], rectCenter, [], [], [], []);
    vbl = Screen('Flip', window, vbl + (waitframes - 0.5) * ifi);
end
