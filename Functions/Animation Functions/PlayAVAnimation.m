function [vbl,respMade,rt] = PlayAVAnimation(AnimationTextures, AudioMatrix, pahandle, volume, window, vbl, ifi, GetResp, tStart, PreviousRespMade, rt, rectCenter)
...
if vbl == 0
    ...
    for frame = 1:timeFrames
        Screen('DrawTexture', window, AnimationTextures(frame), [], rectCenter, [], [], [], []);
        vbl = Screen('Flip', window, vbl + (waitframes - 0.5) * ifi);
        if GetResp == true
            %detecting response
            KeyIsDown = KbCheck;
