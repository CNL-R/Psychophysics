...
%No probability gaussian on gabors. 
...
%   Row 1: block type
%   Row 2: 1 for subliminal catch trial, 2 for a supraliminal catch trial, 0 for not a catch trial
%   Row 3: Coherence
...
% %--------------------
%         timesCue(trial + 1, :) = NaN(1, blocks);
%         timesCue(trial + 2, :) = mean(timesCue(1:trials, :));
%         timesCue(trial + 3, :) = std(timesCue(1:trials, :));
%         
%         timesISI(trial + 1, :) = NaN(1, blocks);
%         timesISI(trial + 2, :) = mean(timesISI(1:trials, :));
%         timesISI(trial + 3, :) = std(timesISI(1:trials, :));
%         
%         timesPostP(trial + 1, :) = NaN(1, blocks);
%         timesPostP(trial + 2, :) = mean(timesPostP(1:trials, :));
%         timesPostP(trial + 3, :) = std(timesPostP(1:trials, :));
%         
...
filepathTiming = 'C:\Users\lhshaw\Desktop\Psychophysics DATA';
filename = [participant '.mat']; 
filenameTiming = [participant '_timing' '.mat'];

save(fullfile(filepath, filename), 'blockMatrix','historyMatrix','stimMat', 'respMatrix', 'historyPRespMatrix', 'historyNRespMatrix', 'historyCatchMatrix', 'gaborMatrix');
save(fullfile(filepathTiming, filenameTiming), 'timesCue','timesISI', 'timesPostP', 'timesPreP', 'timesPreStim', 'timesPresentation', 'timesStim');
