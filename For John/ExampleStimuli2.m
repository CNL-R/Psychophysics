%% INITIAL SETUP CODE
%ExampleStimuli2 tries to make a better looking gabor by setting outskirts of gauss window to NaN and setting all NaNs in gabor to noise. 
...
coherence = .25;
annulus = 0; %Insert annulus? 1 for yes. 0 for no. 
...
appX = 500; %apperature size (diameter of the circle in pixels)
appRadius = appX/2;
appY = appX;
appXCenter = appX/2;
appYCenter = appY/2;
...
gabor = CreateGabor2(stimLength, sigma, lambda, 'r', 'r', A);
gabor = EmbedInNoise2(gabor, coherence, probabilityGaussian, gauss);
gaborMatrix = EmbedInApperature(gabor, 0, appX, appY, 'n', 0.5);
if annulus == 1
