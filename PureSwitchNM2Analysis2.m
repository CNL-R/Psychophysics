...
ncondsSeparate = 17;                                    %number of separate analysis conditions, 1 = VVV, 2 = AVV, 3 = AAA, 4 = VAA
...
for j = 1:size(id,2)                                   %loop iterating through individual data directories
    ...
    for k = 1:ncondsSeparate                                     %@calculate the mean reaction times across participants for each Separate analysis condition (VVV, AAA, AVV, VAA, etc..)
        indcondidxSeparate = find(nm2indpossible(:,4)==k);
        inddataSeparate(j,k,:) = [mean(nm2indpossible(indcondidxSeparate,2)) std(nm2indpossible(indcondidxSeparate,2)) length(indcondidxSeparate)]; %store the mean reaction times for each participant for each condition in the inddata array
    end
...
for i = 1:ncondsSeparate                                   %for every condition
    nm2allcondidx = find(nm2indpossible(:,4)==i);
    nm2allmeandata(i,:) = [mean(nm2indpossible(nm2allcondidx,2)) std(nm2indpossible(nm2allcondidx,2)) length(allcondidx)]; %take the mean of the data for each condition (first column), the SD (second column, and give the n (third)
end

outputSeparate = nm2allmeandata;
