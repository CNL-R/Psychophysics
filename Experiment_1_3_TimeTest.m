%Changes in 1_3: 
% - Gathering responses from cues
% - InterCueStimulus Interval set equal to ISI.
% - Gabor Stimulus Patch Reoptimized. Made Edges of high coherence patches more smooth by allowing for the sigma of both the gabor's gaussian window and probability
%       gaussian. 
...
%TIMING VERSION of EXPERIMENT 1_3
...
%probability gaussian properties
sigma_prob = 80;
s_prob = sigma_prob/imSize;
gauss_prob = exp( -(((Xm.^2)+(Ym.^2)) ./ (2* s_prob^2)));
...
gabor = EmbedInNoise2(gabor, initialCoherence, 1, gauss_prob);
...
preStimTime = [1000 1000]; %[1000 2140]; TIMING CODE
...
%3 - RT to stimulus if detected. 0 if not detected. Row 4 - 1 or 0 for if cue was responded to or not. Row 5 - RT to cue if detected. 0 if Not detected. Each column is an individual
%stimulus presentation. Third dimmension is block number
respMatrix = nan(5, prealNum, blocks);
...
%   Row 5: RT to Stimulus
%   Row 6: 1 for positive sign. -1 for negative sign.
%   Row 7: 1 for respMade to cue 0 for no RespMade to cue
%   Row 8: RT to Cue
historyMatrix = NaN(7,prealNum, blocks);
...
for block = 1:blocks
    ...
    %matrix to hold RTs for supraliminal catches and cues
    supraRT = [];
    cueRTs = [];
...
%              KbStrokeWait;
        ...
        if blockMatrix(1, block) == 1
           ...
           [vbl,cueRespMade,cueRT] = PresentMoreEfficientAVStimulus(cueTexture, 'nullCue.wav', pahandle, 0, window, vbl, ifi, cueTime, 0, true, cueStart, cueRespMade, cueRT, rectCenter);
        elseif blockMatrix(1, block) == 2
            %presenting pure audio cue
           [vbl,cueRespMade,cueRT] = PresentMoreEfficientAVStimulus(annulusTexture, 'auditoryCue.wav', pahandle, 0.5, window, vbl, ifi, cueTime, 0, true, cueStart, cueRespMade, cueRT, rectCenter);
        elseif blockMatrix(1, block) == 3
            %presenting AV cue
           [vbl,cueRespMade,cueRT] = PresentMoreEfficientAVStimulus(cueTexture, 'auditoryCue.wav', pahandle, 0.5, window, vbl, ifi, cueTime, 0, true, cueStart, cueRespMade, cueRT, rectCenter);
        end
        cueRespMade %DEBUGGING CODE
        cueRT %DEBUGGING CODE
        ...
        %saving stimulus response to respMatrix
        %stimulus #
        respMatrix(1, trial, block) = stimMat(1, trial, block);
        %1 or 0 for whether or not a response was made to sitmulus
        if respMade == true
        ...
        %1 or 0 for whether or not a response was made to the cue
        if cueRespMade == true           
            respMatrix(4, trial, block) = 1;
            historyMatrix(7, trial, block) = 1;
        else
            respMatrix(4, trial, block) = 0;
            historyMatrix(7, trial, block) = 0;
        end 
        
        %assigning rt
        respMatrix(3, trial, block) = rt;
        historyMatrix(5, trial, block) = rt;
        respMatrix(5, trial, block) = cueRT;
        historyMatrix(7, trial, block) = cueRT;
    ...
    while trial <= trials
        ...
        gabor = CreateGabor(stimLength, sigma, lambda, 'r', 'r', A);
        gabor = EmbedInNoise2(gabor, coherence, 1, gauss_prob);
...
blockMatrix(4, block) = mean(cueRTs);
...
%--------------------
%filesaving
%filepath = 'C:\Users\lhshaw\Desktop\Psychophysics DATA'; %Presentation Computer
filepath = 'C:\Users\achen52\Desktop\Data'; %Coding Computer
